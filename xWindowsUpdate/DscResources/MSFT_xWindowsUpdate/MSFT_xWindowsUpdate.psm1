...
function Set-TargetResource
{
    ...
    if($Ensure -eq 'Present')
    {
		...
		# Some hotfix do not show up in Get-Hotfix. Create file with hotfix name to test it later
		else {
			echo "KB$kbId" >> "$env:SystemDrive:\KB$kbId"
		}
...
function Test-TargetResource
{
    ...
    if($Ensure -eq 'Present')
    {
		# Some hotfix do not show up in Get-Hotfix. Check if file with hotfix name is created prior, which means hotfix is installed
		if($returnValue -eq $false){
			$returnValue = [bool](Test-Path "$env:SystemDrive:\KB$kbId")
		}
